...
int main(){
    ...
    for( int i = 0; i <= 20; i++){
        ...
        // first check that the effective dM^2 from the tensor solver is what we expect
        ...
        TEST_EXPECTED(effDm2, bargerProp.calculateEffectiveDm2(energy), "effective dM^2 for theta == " << theta, 0.00001)
        ...
        TEST_EXPECTED(PMNSeff.getValue<float>({0, 0,0}), bargerProp.getPMNSelement(energy, 0, 0), "PMNS[0,0] for theta == " << theta, 0.00001)

        TEST_EXPECTED(PMNSeff.getValue<float>({0, 1,1}), bargerProp.getPMNSelement(energy, 1, 1), "PMNS[1,1] for theta == " << theta, 0.00001)
        
        TEST_EXPECTED(PMNSeff.getValue<float>({0, 0,1}), bargerProp.getPMNSelement(energy, 0, 1), "PMNS[0,1] for theta == " << theta, 0.00001)
        
        TEST_EXPECTED(PMNSeff.getValue<float>({0, 1,0}), bargerProp.getPMNSelement(energy, 1, 0), "PMNS[1,0] for theta == " << theta, 0.00001)
