...
int main(){
    ...
    // check that we get no vacuum oscillations when theta == 0 for a range of energies
    bargerProp.setParams(/*m1=*/1.0, /*m2=*/2.0, /*theta=*/0.0, baseline);
    for(int i = 0; i < 100; i++){
        float energy = (float)i * 10.0; 

        if(
            relativeDiff(bargerProp.calculateProb(energy, 0, 0), 1.0) > 0.00001 
        ...
        {
            std::cerr << "ERROR: probabilities for theta == 0 should be identity matrix" << std::endl;
    ...
    // check that we get no vacuum oscillations when m1 == m2 for a range of energies
    ...
    bargerProp.setParams(/*m1=*/1.0, /*m2=*/2.0, /*theta=*/M_PI/8.0, /*baseline=*/4.0);
