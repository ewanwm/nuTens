...
int main()
{
    NT_PROFILE_BEGINSESSION("two-flavour-const-matter-anti-nu-test");
    ...
    for (int i = 0; i <= 20; i++)
    {
        float theta = (-1.0 + 2.0 * (float)i / 21.0) * 0.48 * M_PI;

        std::cout << "######## theta = " << theta << " ########" << std::endl;

        bargerProp.setParams(m1, m2, theta, baseline, density, /*antiNeutrino=*/false);

        std::cout << "lm() nu:         " << bargerProp.lm() << std::endl;
        std::cout << "ang  nu:         " << bargerProp.calculateEffectiveAngle(energy) << std::endl;
        std::cout << "dm2  nu:         " << bargerProp.calculateEffectiveDm2(energy) << std::endl;
        std::cout << "off-diag nu:     " << bargerProp.calculateProb(energy, 0, 1) << std::endl << std::endl;

        bargerProp.setParams(m1, m2, theta, baseline, density, /*antiNeutrino=*/true);

        std::cout << "lm() antinu:     " << bargerProp.lm() << std::endl;
        std::cout << "ang  antinu:     " << bargerProp.calculateEffectiveAngle(energy) << std::endl;
        std::cout << "dm2  antinu:     " << bargerProp.calculateEffectiveDm2(energy) << std::endl;
        std::cout << "off-diag antinu: " << bargerProp.calculateProb(energy, 0, 1) << std::endl;
        ...
        std::cout << std::endl << "Oscillation probs: " << std::endl;
