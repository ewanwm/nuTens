...
option(
    NT_USE_TORCH 
    "Use torch as the backend for dealing with tensors"
    ON
)

option(
    NT_TORCH_FROM_PIP 
    "If it is not found, torch will be installed using pip"
    ON
)

option(
    NT_ALLOW_GLOBAL_PYTHON_ENV 
    "Allow installing pip packages in global python environment"
    OFF
)
...
option(
    NT_COMPILE_TESTS 
    "whether or not to compile unit and integration tests"
    ON
)
...
option(
    NT_ENABLE_BENCHMARKING 
    "enable benchmarking using google benchmark"
    OFF
)
mark_as_advanced(NT_ENABLE_BENCHMARKING)

option(
    NT_ENABLE_GPU_BENCHMARKING 
    "enable benchmarking on GPU"
    OFF
)
...
option(
    NT_TEST_COVERAGE 
    "produce code coverage reports when running tests"
    OFF
)
...
option(
    NT_BUILD_TIMING 
    "output time to build each target"
    OFF
)
